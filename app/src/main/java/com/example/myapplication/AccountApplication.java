...
public class AccountApplication extends Application {
    ...
    public static HashMap<String, String> mUserIdNameMap = new HashMap();  //放置 an 安静 等用户，"user_id":"an","user_name":"安静"
    public static HashMap<String, String> mUserNameIdMap = new HashMap();  //放置 安静 an  等用户，"user_id":"an","user_name":"安静"

    private static AccountApplication mAccountApplication;
    public static String mDefaultUserName;  //默认的用户名，如安静
    public static String mDefaultGetUserName;  //默认的用户名，如安静
    ...
    public void initUserMap() {
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    final String usersResult = Utils.doGet(Utils.getUsersPath());
                    Log.i(TAG, "getUserMap run(),usersResult=" + usersResult);
                    JSONArray jsonArray = new JSONArray(usersResult);
                    for (int i = 0; i < jsonArray.length(); i++) {
                        JSONObject jsonObject = jsonArray.getJSONObject(i);
                        String user_name = jsonObject.getString("user_name");
                        mUserIdNameMap.put(jsonObject.getString("user_id"), user_name);  // an 安静
                        mUserNameIdMap.put(user_name, jsonObject.getString("user_id"));  // an 安静
                        if (i == 0) {
                            mDefaultUserName = user_name;
                        } else if (i == 1) {
                            mDefaultGetUserName = user_name;
                        }
                    }
                } catch (Exception e) {
                    Log.i(TAG, "catch" + e);
    ...
    public void initTypeMap() {
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    final String result = Utils.doGet(Utils.getTypesPath());
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        Log.i(TAG, "initTypeMap(),i=" + i + ",type_name=" + type_name);
                    }
                } catch (Exception e) {
                    Log.i(TAG, "catch" + e);
                    e.printStackTrace();
